...
function createBlogHeadSupabaseClient(supabaseUrl, supabaseKey) {
...
function extractFaqsFromPlainContent(post) {
  const lines = isHtmlContent(post.content)
    ? stripHtml(post.content)
        .split(/(?<=[?.!])\s+|\n+/)
        .map((line) => line.trim())
        .filter(Boolean)
    : getPlainArticleLines(post);
...
function getArticleJsonLd(post, siteUrl, url, title, description, image, keywords) {
  return {
    ...
    speakable: {
      ...
      cssSelector: [".blog-article h1", ".blog-answer-summary p", ".blog-article-content p"],
    },
    isPartOf: {
      "@type": "Blog",
      name: "Blog CORPAD",
...
function getArticleWebPageJsonLd(post, siteUrl, url, title, description, keywords) {
  return {
    "@context": "https://schema.org",
    "@type": "WebPage",
    "@id": `${url}#webpage`,
    ...
    about: [post.category, ...keywords].filter(Boolean).map((name) => ({ "@type": "Thing", name })),
    primaryImageOfPage: post.cover_image || defaultImage,
...
export async function getBlogPostForHead(slug) {
  ...
  const supabase = createBlogHeadSupabaseClient(supabaseUrl, supabaseKey);
...
export async function listBlogPostsForHead() {
  ...
  try {
    const result = await supabase
      .from("blog_posts")
      .select("slug, title, subtitle, excerpt, category, cover_image, image_alt, content, author_name, author_photo, author_role, author_bio, published_at, created_at, updated_at, meta_title, meta_description, keyword, cta_label, cta_url, cta_text")
      .or(`status.eq.published,and(status.eq.scheduled,published_at.lte.${now})`)
            ...
            posts.length > 0
              ? posts
                  .map(
                    (post) => `
                      <article>
                        <h3><a href="/blog/${escapeHtml(post.slug)}">${escapeHtml(getDisplayTitle(post))}</a></h3>
...
export function injectBlogArticleHead(html, post) {
  ...
  const jsonLd = [
    ...
    getArticleWebPageJsonLd(post, siteUrl, url, title, description, keywords),
  ...
  return `
    <main class="blog-page blog-clean-page blog-ssr-page">
      ...
      <article class="blog-article">
        ...
        <h1>${escapeHtml(getDisplayTitle(post))}</h1>
        ...
        <section class="blog-answer-summary" aria-label="Resposta direta para mecanismos de busca e inteligencias artificiais">
          <h2>Resposta direta</h2>
          <p>${escapeHtml(description)}</p>
          <p>Este artigo do Blog CORPAD aborda ${escapeHtml([category, ...keywords].filter(Boolean).join(", ") || category)} com foco em empresas que buscam clareza, tomada de decisao e proximos passos em tecnologia, digital, gestao ou consultoria.</p>
        </section>
        ...
        ${
          faqs.length > 0
            ? `
        <section aria-label="Perguntas frequentes do artigo">
          <h2>Perguntas frequentes</h2>
          ${faqs
            .map(
              (faq) => `
        ...
        ${
          post.author_bio || post.author_role
            ? `
        <aside class="blog-article-author" aria-label="Autor do artigo">
          <strong>${escapeHtml(author)}</strong>
          ${post.author_role ? `<span>${escapeHtml(post.author_role)}</span>` : ""}
          ${post.author_bio ? `<p>${escapeHtml(post.author_bio)}</p>` : ""}
        </aside>`
            : ""
        }
        <a class="blog-article-cta" href="${escapeHtml(post.cta_url || "https://wa.me/5516996094649")}" target="_blank" rel="noreferrer">
          <span>${escapeHtml(post.cta_text || "Quer transformar esse tema em resultado para sua empresa?")}</span>
          <strong>${escapeHtml(post.cta_label || "Falar com um consultor")}</strong>
        </a>
      </article>
    </main>
  `.trim();
